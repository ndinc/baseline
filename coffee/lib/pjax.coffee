class Pjax
  ...
  setAnchor: ()->
    ...
    $anchor.each (i, el) =>
      url = $(el).attr("href")
      if @isSelfDomain(url) and url.match(/jpg|jpeg|png|gif/g) == null and url.indexOf('javascript:') == -1 and url != '#'
        anchors.push el
    @$anchors = $(anchors)
  ...
  loadContent: (url, isPopStatsEvent, callback) ->
    ...
    $.when(
      ...
      window.history.pushState null, title, url unless isPopStatsEvent
      @$title.html title
